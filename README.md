...
Replace ```<EVIDENCE_FILE>``` with the path to your evidence file and ```<PROTEOME_FILE>``` with the path to a protein database containing all protein accessions included in the evidence file. You can find more detailed information about the input data [here](#input).  
...
| [🔴 qc] | If the two QC plots [consensus_sequence_coverage.png](#consensus_sequence_coveragepng) and [intern_extern.svg](#intern_externsvg), are generated. |
