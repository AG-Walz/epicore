...
def plot_protein_landscape(protein_df: pd.DataFrame, accession: str, proteome_dict: dict[str,str]) -> plt.figure:
    ...
    for group, landscape in enumerate(prot_row['landscape'].iloc[0]):
        ...
        for idx, position in enumerate(landscape):
            ax.bar(group_start+int(idx),position,width=1, alpha=0.4, color=color) # plot landscape
        for pos in range(prot_row['core_epitopes_start'].iloc[0][group],prot_row['core_epitopes_end'].iloc[0][group]+1):
            ax.bar(pos,0.5,width=1,color=color) # plot consensus sequence of peptide group
...
def included(start: int, end: int, overlap_df: pd.DataFrame , included_df: pd.DataFrame) -> bool:
...
def get_largest_overlap(starts: list[int], ends: list[int], pep_start: int, pep_end: int, intern: bool) -> int:
    ...
    for start, end in zip(starts, ends):
        ...
        if start <= pep_start: # peptide occurs before the current
            max_overlap = max(min(pep_end-pep_start+1,end-pep_start+1), max_overlap)
...
def get_minimal_overlap(starts: list[int], ends: list[int], pep_start: int, pep_end: int) -> int:
...
def consensus_coverage(start: int, end: int, consensus_start: int, consensus_end: int) -> int:
    ...
    if start <= consensus_start: 
        return max(0,min(end-consensus_start+1, consensus_end-consensus_start+1))/consensus_len
    else:
        return max(0,min(consensus_end-start+1,end-start+1))/consensus_len


def build_overlap_df(starts: list[int], ends: list[int]) -> tuple([pd.DataFrame, pd.DataFrame]):
...
def all_overlap(row: pd.Series) -> pd.DataFrame:
    ...
    pep = pd.DataFrame(columns=['previous','intern_max','intern_min','next','len_pep','len_core','sequence','consensus_sequence_coverage','included'])
    k = 0

    # iterate all peptide groups
    for i, (group_starts, group_ends, group_sequences, consensus_start, consensus_end) in enumerate(zip(row['grouped_peptides_start'],row['grouped_peptides_end'], row['grouped_peptides_sequence'], row['core_epitopes_start'], row['core_epitopes_end'])):
        ...
        for _, (start, end, sequence) in enumerate(zip(group_starts, group_ends, group_sequences)):

            # calculate maximal overlap to previous group
            if i > 0:
                pep.loc[k,'previous'] = get_largest_overlap(row['grouped_peptides_start'][i-1], row['grouped_peptides_end'][i-1], start, end, False)
            else:
                pep.loc[k,'previous'] = 0

            # calculate maximal overlap to next group
            if i < len(row['grouped_peptides_start'])-1:
                pep.loc[k,'next'] = get_largest_overlap(row['grouped_peptides_start'][i+1], row['grouped_peptides_end'][i+1], start, end, False)
            else:
                pep.loc[k,'next'] = 0

            # calculate the maximal and minimal overlap within a group 
            pep.loc[k, 'intern_max'] = get_largest_overlap(group_starts, group_ends, start, end, True)
            pep.loc[k, 'intern_min'] = get_minimal_overlap(group_starts, group_ends, start, end)
            pep.loc[k,'len_pep'] = end-start+1
            pep.loc[k,'len_core'] = consensus_end-consensus_start+1
            pep.loc[k,'sequence'] = sequence
            pep.loc[k,'consensus_sequence_coverage'] = consensus_coverage(start, end, consensus_start, consensus_end)
...
def qc_plots(protein_df: pd.DataFrame, out_dir: str):
    ...
    # calculate the overlap of a peptide with the previous, current and next peptide group
